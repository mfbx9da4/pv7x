...
function getDayColor(day: DayInfo): string {
	...
	if (day.passed)
		return getCssVar(
			day.isOddWeek ? "--color-passed-odd" : "--color-passed-even",
		);
	return getCssVar(
		day.isOddWeek ? "--color-text-tertiary" : "--color-text-secondary",
	);
}
...
function getBorderColor(day: DayInfo): string {
	...
	if (day.color === "subtle" && day.passed) {
		return getCssVar(
			day.isOddWeek
				? "--color-milestone-passed-odd"
				: "--color-milestone-passed-even",
		);
	}
...
function getAnnotationTextColor(day: DayInfo): string {
	...
	if (day.color && TEXT_COLOR_OVERRIDES.includes(day.color)) {
		return (
			getCssVar(`--color-${day.color}-text`) ||
...
export function Tooltip({
	...
	const description = day.annotation
		? annotationDescriptions[day.annotation]
		: null;
