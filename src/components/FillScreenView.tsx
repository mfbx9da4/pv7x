...
function calculateGrid(
	...
	for (let cols = 1; cols <= totalDays; cols++) {
		...
		if (
			empty < bestEmpty ||
			(empty === bestEmpty && aspectDiff < bestAspectDiff)
		) {
			bestEmpty = empty;
			...
			{days.map((day) => (
				...
				>
					{day.annotation ? (
						cellSize >= 50 ? (
							<>
								<span class="date-label" style={{ fontSize: `${fontSize}px` }}>
									{formatDate(addDays(startDate, day.index))}
								</span>
								<AnnotationText
									text={day.annotation}
							...
							>
								...
								/>
								<span
									class={`annotation-date ${showAnnotationDate ? "visible" : "hidden"}`}
								>
									{formatDate(addDays(startDate, day.index))}
								</span>
							</span>
